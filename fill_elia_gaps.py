...
# -*- coding: utf-8 -*-
"""
Fill missing QHs in forecasters/elia_forecaster/forecast_log_full.csv.

For each missing QH between 2025-01-01 and today:
  1. Load / download SI 1-min from ODS133 (reuses data/backfill_elia/ cache)
  2. Run extract_features() + model.predict()
  3. Fetch actual_volume + actual_isp from ODS134 (QH resolution)
  4. Append, deduplicate, sort, save.
"""
...
def _download_ods133_day(date_str: str, out_path: Path) -> int:
    ...
    start_utc = dt.datetime(d.year, d.month, d.day,
            ...
            if dt_raw and si_val is not None:
                try:
                    ts = (pd.to_datetime(dt_raw, utc=True)
                            .tz_convert(BRUSSELS).tz_localize(None))
...
def load_si_for_dates(dates_needed: set) -> pd.DataFrame:
    SI_DIR.mkdir(parents=True, exist_ok=True)
    frames = []
    total = len(dates_needed)
    for i, d in enumerate(sorted(dates_needed)):
        date_str = d.isoformat()
        out = SI_DIR / f"si_1min_{date_str}.csv"
        if not (out.exists() and out.stat().st_size > 10_000):
            log.info("  [%d/%d] Downloading ODS133 %s ...", i + 1, total, date_str)
...
def fetch_ods134_actuals_chunk(start_dt, end_dt) -> pd.DataFrame:
    ws = (pd.Timestamp(start_dt)
            .tz_localize(BRUSSELS, nonexistent="shift_forward")
            .tz_convert("UTC").strftime("%Y-%m-%dT%H:%M:%SZ"))
    we = (pd.Timestamp(end_dt)
            .tz_localize(BRUSSELS, nonexistent="shift_forward")
            .tz_convert("UTC").strftime("%Y-%m-%dT%H:%M:%SZ"))
    ...
    return df.drop_duplicates("forecast_time")


def fetch_ods134_actuals(start_dt, end_dt, chunk_days=60) -> pd.DataFrame:
    frames = []
    cursor = pd.Timestamp(start_dt)
    end_ts = pd.Timestamp(end_dt)
    while cursor < end_ts:
        chunk_end = min(cursor + pd.Timedelta(days=chunk_days), end_ts)
        chunk = fetch_ods134_actuals_chunk(cursor, chunk_end)
        if not chunk.empty:
            frames.append(chunk)
        cursor = chunk_end
        time.sleep(0.5)
    if not frames:
        return pd.DataFrame(columns=["forecast_time", "actual_volume", "actual_isp"])
    return (pd.concat(frames, ignore_index=True)
...
def main():
    ...
    expected = pd.date_range("2025-01-01", today, freq="15min")
    present  = set(df["forecast_time"])
    missing  = [t for t in expected if t not in present]
    log.info("Missing QHs: %d  (%s → %s)",
             len(missing), missing[0] if missing else "-", missing[-1] if missing else "-")

    if not missing:
        log.info("Nothing to do.")
        return

    # Dates needed for SI 1-min (include previous day for early-hour feature windows)
    dates_needed: set = set()
    for qh in missing:
        dates_needed.add(qh.date())
        if qh.hour == 0:
            dates_needed.add((qh - pd.Timedelta(days=1)).date())
    log.info("SI 1-min needed for %d dates ...", len(dates_needed))
    si_all = load_si_for_dates(dates_needed)
    log.info("SI 1-min loaded: %d rows", len(si_all))

    # ODS134 actuals (in 60-day chunks)
    log.info("Fetching ODS134 actuals (%s → %s) ...",
             missing[0].date(), missing[-1].date())
    ...
    for i, qh in enumerate(missing):
        ...
        if (i + 1) % 500 == 0:
            log.info("  [%d/%d] ok=%d err=%d", i + 1, len(missing), n_ok, n_err)
    ...
    has_pmin  = full["price_min"].notna().sum() if "price_min" in full.columns else 0
    remaining = pd.date_range("2025-01-01", today, freq="15min").difference(full["forecast_time"])

    log.info("=" * 55)
    log.info("FINAL COVERAGE")
    log.info("  Total rows      : %d", total)
    log.info("  forecast_value  : %d (%.1f%%)", has_fcast, has_fcast / total * 100)
    log.info("  price_min       : %d (%.1f%%)", has_pmin,  has_pmin  / total * 100)
    log.info("  Still missing   : %d QHs", len(remaining))
    log.info("=" * 55)
